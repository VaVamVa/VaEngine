...
구현 요청 시 코드를 쓰기 전에 이 원칙을 먼저 확인한다. 작업 태도는 [`Agent/context.md`](Agent/context.md)의 "작업 규칙"도 함께 따른다.

### 프로젝트 성격

VaEngine은 빠른 상품화가 목적이 아니라, **어떤 최적화가 가능하고 그 최적화가 언제 필요한지를 비교·수치로 보여주는 포트폴리오**다. 따라서 구현 방식을 고를 때 "일단 동작하면 충분"을 기준으로 삼지 않는다 — 여러 선택지가 있다면 하나로 좁히기보다 트레이드오프가 드러나도록 남겨두고, 성능·메모리 등 수치로 비교 가능한 형태로 구현하는 쪽을 우선한다. 아래 "구현 전 필수 확인"·"기술 부채는 구현 전에 고지" 원칙은 이 성격에서 나온다.
