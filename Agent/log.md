...
4. 위의 내용을 `[GitRoot]/AgentLog/YYYY-MM-DD_Log.md`에 추가
    - `YYYY-MM-DD`는 현재 시스템 날짜 ( UTC+09:00 )
...
1. "`요약` 및 요약 관련 요청" 이 들어오면 최근 Start Log 또는 이전 Compact Log 시점부터 현재까지의 작업을 요약
    - **중요**: Start Log가 작성된 파일(예: AgentLog/{이전_작성_날짜}.md)에 새로운 Compact Log를 추가
...
1. "`종료` 및 종료 관련 요청" 이 들어오면 [중간_요약](./log.md/#agent-중간-요약)을 참고하여 Compact Log 작성
    - 만약 이전 Compact 부터 변경 사항이 없다면 Skip

2. Start Log 시점부터 현재까지의 일을 간략하게 요약하여 해당 파일에 End Log 추가
    - **중요**: Start Log가 작성된 파일(예: AgentLog/{이전_작성_날짜}.md)에 End Log를 추가
    - 만약 날짜가 바뀌어도 현재 진행 중인 Start Log가 있는 파일에 계속 기록
    - 제목과 현재 시각을 log 시작지점에 기입
